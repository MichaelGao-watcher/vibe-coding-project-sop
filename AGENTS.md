...
> **目录结构**：本骨架根目录只保留规则文件（`AGENTS.md`、`README.md`、`skeleton-manifest.json`）。所有模板文件存放在 `templates/` 子目录下。消费时必须按规则从 `templates/` 按需复制，禁止直接复制整个目录。
...
| `status.md` | 当前进度、待办清单（从 `templates/` 复制模板） |
| `session-log.md` | 会话历史记录（从 `templates/` 复制模板） |
| `decisions.md` | 关键决策记录（从 `templates/` 复制模板） |
| `troubleshooting.md` | 问题索引（从 `templates/` 复制模板） |
| `lessons-learned.md` | 跨项目经验沉淀（从 `templates/` 复制模板） |
...
1. **先建立基础设施层**：从骨架 `templates/` 目录复制/初始化上述 7 个文件到项目根目录
