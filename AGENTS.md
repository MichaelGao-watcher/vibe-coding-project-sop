...
**标准动作序列**（用户确认后执行）：
1. 对每个存活项目运行 `python scripts/pull.py --skeleton <母库路径> --force`（同步知识文件）
2. 对每个项目在 AGENTS.md 核心约束表中新增 RULE-08（已有则检查措辞是否对齐）
3. 复制 `build-experience-index.py` 到每个项目的 `scripts/` 目录
4. 在每个项目中运行 `python3 scripts/build-experience-index.py` 生成索引
5. 对每个项目 Git commit + push
6. 汇报分发结果
