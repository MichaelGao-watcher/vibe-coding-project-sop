...
| `status.md` | 现在在哪、下一步去哪 | **每次会话结束后** | "现在在哪？" |
| `session-log.md` | 怎么走到这里的 | **每次会话结束后追加** | "Context 丢失后怎么恢复？" |
| `decisions.md` | 为什么这样设计 | 做关键决策时 | "为什么选 A 而不是 B？" |
| `troubleshooting.md` | 遇到问题怎么救 | 踩坑并解决后 | "这个报错怎么修？" |
| `lessons-learned.md` | 什么可以跨项目复用 | 发现可复用经验时 | "下个项目怎么避免？" |
...
时机：代码提交/测试通过 → 更新 `status.md` → **追加 `session-log.md`**。
